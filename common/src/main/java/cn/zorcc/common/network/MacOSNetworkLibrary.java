...
public final class MacOSNetworkLibrary implements OsNetworkLibrary {
    ...
    public void ctl(Mux mux, Socket socket, int from, int to) {
        ...
        int r1 = from & OsNetworkLibrary.REGISTER_READ, r2 = to & OsNetworkLibrary.REGISTER_READ;
        if(r1 != r2) {
            checkInt(TenetMacosBinding.keventCtl(kqfd, fd, Constants.EVFILT_READ, r1 > r2 ? Constants.EV_DELETE : Constants.EV_ADD), "kevent_ctl");
        }
        int w1 = from & OsNetworkLibrary.REGISTER_WRITE, w2 = to & OsNetworkLibrary.REGISTER_WRITE;
        if(w1 != w2) {
    ...
    public void closeSocket(Socket socket) {
        checkInt(TenetMacosBinding.close(socket.intValue()), "close socket");
    }

    @Override
    public void shutdownWrite(Socket socket) {
        checkInt(TenetMacosBinding.shutdownWrite(socket.intValue()), "shutdown write");
    }
