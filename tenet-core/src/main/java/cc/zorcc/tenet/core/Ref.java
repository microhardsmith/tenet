...
public final class Ref {
    ...
    public Object fetch() {
        ...
        for( ; ; ) {
            ...
            if(current == DEFAULT) {
                ...
                throw new TenetException(ExceptionType.MISUSE, "Ref couldn't be reused");
            }else {
                if(cas(current, CONSUMED)) {
                    return current;
                }else {
                    throw new TenetException(ExceptionType.MISUSE, "Multiple fetcher detected, which should never happen");
                }
            }
        }
    }
    ...
    public void assign(Object result) {
        if(result instanceof Thread) {
            throw new TenetException(ExceptionType.MISUSE, "Could not assign thread to ref as result");
        }
        for( ; ; ) {
            Object current = getVolatileValue();
            if(current == DEFAULT) {
