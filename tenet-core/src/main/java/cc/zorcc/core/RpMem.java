...
import cc.zorcc.core.bindings.TenetBinding;
...
public final class RpMem {
    ...
    public static Mem acquire() {
        ...
        if(currentThread.isVirtual()) {
            throw new FrameworkException(ExceptionType.NATIVE, "Shouldn't be acquiring rpMalloc in virtual thread");
        }
        lock.lock();
        try {
    ...
    public static void release() {
        Thread currentThread = Thread.currentThread();
        if(currentThread.isVirtual()) {
            throw new FrameworkException(ExceptionType.NATIVE, "Shouldn't be releasing rpMalloc in virtual thread");
        }
        lock.lock();
        try {
