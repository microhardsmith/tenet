    ...
    record HeapWriteBufferPolicy(
        ...
        public MemorySegment resize(MemorySegment segment, long currentIndex, long nextIndex) {
            if(segment == MemorySegment.NULL) {
                ...
                byte[] b1 = (byte[]) segment.heapBase().orElseThrow(() -> new FrameworkException(ExceptionType.NATIVE, "Not a heap segment"));
                byte[] b2 = (byte[]) newSegment.heapBase().orElseThrow(() -> new FrameworkException(ExceptionType.NATIVE, "Not a heap segment"));
